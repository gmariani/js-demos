...
cv.controls.Accordion = function (element, spacing) {
    /**
...
cv.controls.Accordion.prototype = {
    ...
    selectIndex: function (idx) {
        ...
        if (item) {
            while (i--) {
                if (this.toggleNodes[i] === item) {
                    TweenLite.to(this.contentNodes[i], this.tweenTime, { height: this._arrHeights[i] });
                } else {
                    TweenLite.to(this.contentNodes[i], this.tweenTime, { height: 0 });
                }
            }
            ...
            trace("Accordion - Error: Can't find toggler");
            return false;
        }
    },
};
